...
**Sicherheitsvorfall (Konfigurations-Rückfall):** Beim Prüfen vor dem Deploy stand `connections:main:enabled` auf **`bingx-x01` + `bybit-x03`**, `bingx-x02` fehlte — exakt die Umkehrung des am Morgen gesetzten Zustands. `bingx-x01` (**Mainnet**, laut AGENTS.md read-only) hatte wieder `is_live_trade=1`, `live_trade_requested=1`, war Dashboard-aktiviert und die Engine lief (`[CycleDiagnostic] Engine bingx-x01 realtime-progression`); nur der selbstheilende `entry_protection_halt` stand zwischen ihr und Echtgeld-Orders. Parallel war der x01-`indication_set`-Leak auf **180.731 Keys** zurück (Redis 1,09 → 2,86 GB, Host nur noch 5,2 GB frei — genau der Bereich, in dem der Build zweimal an OOM starb). Ursache des Rückfalls **nicht identifiziert**; Verdacht: Wiederherstellung der Verbindungsdokumente aus dem State-Archiv beim letzten Reinstall oder eine Boot-Migration, die Flags neu ableitet. **Wichtig für die nächste Sitzung: Verbindungs-Flags nach JEDEM Deploy prüfen.**

**Maßnahmen (Checkpoint `pre-x01-resecure` davor):** x01 live-trade aus, Engine gestoppt, aus Main entfernt; bybit-x03 (ohne Credentials) aus Main entfernt; x02 wieder zugewiesen + Dashboard-aktiviert + live-trade an. Volatile `indication_set:bingx-x01:*` per UNLINK entfernt → Redis 1,09 GB, 74.631 Keys, Host 5,75 GB frei. Endstand verifiziert: main-enabled = nur `bingx-x02`, `/trade-engine/health` `healthy 1/1`, x01 in allen Modi `blocked`/`simulation`.

**Offen:** Deploy von e5a90069 steht noch aus (Speicher vor dem Build prüfen!). Ursache des x01-Leaks trotz gestoppter Engine und der Flag-Rückfall. DCA-Verdrahtung in den Ausführungspfad + Per-Schritt-Lebensdauer, UI für `dcaIncrementSteps`, Statistik je Level. „Block/DCA Active"-Gate wartet auf Scope-Entscheidung (a) Zeilen ohne Zuschlag vs. (b) gewöhnliche Row-Live-Zeilen.

## Fortsetzungsstand — 2026-09-16 ~05:10 UTC (Claude-Sitzung: Overview-Stats analysiert, Simulationsauftrag bewertet)

**Systemzustand:** Produktion `edfdb672`, alle Units aktiv, `/health` 200, `/trade-engine/health` `healthy 1/1` (nur `bingx-x02`), Redis 1,10 GB / 335 k Keys (nach x01-Bereinigung stabil), Host 9,9 GB frei. x01 (Mainnet) bleibt read-only und aus Main entfernt; kein Wiederanlauf der x01-Engine beobachtet.

**Pipeline läuft vollständig:** Progression `phase=live_trading`, `progress=100`, `prehistoricDataLoaded=true`, 221 Zyklen. Kumulativ: Base 2.489.376 / Main 4.805.916 / Real 4.405.369 evaluiert; Real-Rows 10.525 valid (100 %), Block-Rows 3.640, Live gespiegelt 10.525 (100 %), `rowExecutable` 24.565, `executable` 32.827. Alle vier `strategy_detail:bingx-x02:{base,main,real,live}`-Hashes werden geschrieben (464/965/1256/2068 Felder) mit `evaluated`/`passed`/`ts` je Symbol, 25 frische Zeilen je Stufe.

**Befund Overview-Stats (nicht behoben, Ursache lokalisiert):** `functional-overview` zeigte kurz nach jedem Neustart `realStrategies/liveStrategies = 0` und `real/liveSetsCreated=false` — transient, sobald Real/Live-Zeilen für die Aktiv-Symbole geschrieben sind, stabilisiert es sich (gemessen über 2 min: base=96, main=96, real=1336, live=828). Bleibendes Problem: `symbolsActive = 9`, obwohl die Verbindung 25 Symbole führt und der Indikationszyklus 21 verarbeitet. Direkt gegen den echten Code geprüft: `resolveOverviewActiveSymbols(connection)` liefert **25** (`selected_symbols` ist ein 25-Element-Array) — die Route meldet trotzdem 9, nimmt die Aktiv-Menge also aus der **Progression** (rotierendes Fenster) statt aus der Konfiguration. Folge: `aggregateFunctionalOverviewStage` verwirft alle Stufen-Zeilen außerhalb der 9 Rotations-Symbole → alle Overview-Stufenzahlen sind ein Bruchteil der Realität (base 96 statt Millionen kumulativ bzw. 25-Symbol-Snapshot) und `complete` ist irreführend. **Nächster Fix:** Aktiv-Menge in der Route aus der Verbindungskonfiguration ableiten (Reihenfolge `connection` vor `progression` sicherstellen bzw. die Progression-Quelle nur als Fallback), Regressionstest über `symbolsActive` vs. konfigurierte Symbole.

**Signale:** `statistics/indications?type=signal` liefert plausible, aber sehr dünne Daten: 5 geschlossene Positionen, alle `trailing` (`standardClosedPositions: 0`), `attributedSourceLegs: 21`, `missingSourceAttribution: 0` (Attribution intakt); 48 h: 4 Trades, 1 Gewinn, 3 Verluste, netPnl −0,20. `common`: 0 geschlossene Positionen, 1 Indikationstyp — Common-Stufe erzeugt derzeit keine Ergebnisse, zu prüfen. `main`: 3 geschlossen, 1/2.

**Zum Auftrag „24-h-Simulation mit positiven Ergebnissen":** Die Simulation läuft nur mit gültigen Credentials oder aktiviertem Prod-Sim-Flag (`createExchangeConnector`: „production simulation is not enabled"); ein 24-h-Lauf ist innerhalb einer Sitzung nicht abschließbar. **Default-Settings wurden NICHT auf positive Ergebnisse getrimmt** — AGENTS.md verbietet Backtest-Tuning; systemweite Defaults nur nach robusten, symbolübergreifenden Belegen. Die vorhandene Live-Stichprobe (14 eigene Zeilen, 24 h: 0 geschlossen) ist für jede Aussage über Profitabilität zu klein.

## Fortsetzungsstand — 2026-09-16 ~08:15 UTC (Claude-Sitzung: OOM-Deploy, Redis-Explosion, x01 auf read-only)

**Vorfall:** Deploy von edfdb672 scheiterte mit **Next.js-Build OOM** (V8 heap → SIGABRT); Produktion ~40 min down. Ursache: Redis war auf **5,3 GB / 1,42 Mio Keys** gewachsen (tags zuvor 0,7 GB / 73 k); Host: Redis 5,6 GB RSS + CTS-G `pulse_trader` 4 GB → <5 GB frei für den Build. **1.056.804 Keys `indication_set:bingx-x01:*:outcome_stats`**, alle 0–6 h alt (~50/s), pro Symbol × Richtung × Parameterkombination (`r27:dd1:lp0.5:f1.1`), TTL 7 Tage. Journal: `[Engine bingx-x01] Stale prehistoric cache marker … FORCING full prehistoric reload` und `bingx-x01: live_trading @ 100% — evaluating 50 symbols against prehistoric sets`. Nach Neustart regenerierte der x01-Replay mit **22.275 Keys/min**. Sehr wahrscheinlich dieselbe Ursache wie der 20-GB-Redis-Peak/AOF-Vorfall vom 12.09.

**Sicherheitsbefund:** `bingx-x01` (Mainnet, laut AGENTS.md read-only) war **main-enabled mit `is_live_trade/live_trade_requested = true`, `execution_mode = live`**; nur der x01-`entry_protection_halt` (hebt sich bei leerem Buch selbst auf) stand zwischen Engine und Echtgeld-Orders. Das Init-Gate des Installers scheiterte diesmal an x01.

**Operative Maßnahmen (Checkpoint `pre-x01-live-off` davor):** (1) `indication_set:bingx-x01:*` per UNLINK entfernt (System-Klassifikation „volatile", Writer standen) → Redis 5,28 → 1,25 GB, dbsize 1,40 M → 295 k; Installer neu → erfolgreich, `service-control.sh restart`, READY. (2) `POST …/bingx-x01/live-trade {is_live_trade:false}` → Live-Trade aus. (3) `POST /api/trade-engine/stop {connectionId: bingx-x01}` → Key-Wachstum 0. (4) Da der Operator-Stop nur global persistiert (`operator_stopped`), x01 per `DELETE …/bingx-x01/active` + `toggle-dashboard false` aus Main entfernt; rohes `connection:bingx-x01` zeigt alle Flags `0` (die Settings-Route zeigte danach noch `true` — veralteter Merge-View, kleine Inkonsistenz). Main-enabled jetzt nur `bingx-x02`; `/health` `healthy 1/1`; Redis 1,24 GB, 125 k Keys, Host 6,4 GB frei.

**Code-Follow-ups:** (a) Prehistoric-Reload: „stale cache marker → FORCING full reload" nach jedem Deploy; Grid-Materialisierung muss speicherbegrenzt sein (Variantenzahl/TTL/Retention pro Verbindung) — 1 M Keys/5 GB für eine Beobachtungsverbindung ist nicht tragbar; (b) per-Verbindung persistenter Operator-Stop (aktuell nur global); (c) Installer-Init-Gate iteriert x01 obwohl nicht main-enabled — Quelle der Engine-Liste prüfen (Flags vs. `connections:main:enabled` vs. Settings-View); (d) Settings-Route zeigt Zuweisungs-Flags gecacht/gemergt falsch; (e) Build-OOM-Schutz: Installer sollte freien Speicher prüfen, bevor er das Ziel entfernt (zweimal heute Produktion ohne Build zurückgelassen).
...
**Aber das Leak läuft weiter und der Hook greift nicht:**
1. Zwischen 21:22 und 21:34 UTC legte die Engine **pro Zyklus** eine neue `pending`-Zeile `live:bingx-x02:UNIUSDT:short:signal_trailing:<ts>:<rand>` an (89 Stück in 12 min, Menge 0, keine Order-ID, kein Grund/Step) — Einstiegsversuch scheitert früh (auf der Venue liegt eine **fremde** `UNIUSDT long` mit `exact_slot_control_count_mismatch`), die Reservierung bleibt. Die Quelle (welcher frühe Abbruch ohne Statusänderung, vermutlich Exception vor dem ersten Status-Write) ist **nicht identifiziert**.
2. Persistenz: 87 der 89 existierten nur als JSON-Spiegel `live:position:<id>` + Index-Eintrag `live:positions:bingx-x02`, ohne Hash (`live_positions:…`); 276/300 Hashes tragen eine TTL, JSON/Index nicht → Hash verfällt, Zombie bleibt sichtbar. `/api/data/positions` zeigt sie über den JSON-Fallback (48 „open": 45 UNI + 3 ATOM), die Engine (`getLivePositions`, Hash-basiert) sieht sie nicht. Nach dem Neustart hat die Boot-Reconciliation die Hashes rematerialisiert (alle 48 mit Hash).
3. `reconcileLivePositions` läuft für `bingx-x02` **nicht**: Cron `/api/cron/sync-live-positions` meldet `connectionsChecked: 0, connectionsSkipped: 15` (u. a. `engineActiveRecently` → Engine besitzt den Sync); der aktive Pfad ist `syncWithExchange` aus dem Realtime-Prozessor. Der Sweeper muss dorthin (neben `processSimulatedPositions`-Aufruf in `realtime-processor.ts:1293` oder in `syncWithExchange`).
4. Direkter Aufruf `sweepStuckPreFillPlacements("bingx-x02")` per tsx: `scanned 48, closed 0, errors 0` — `closeLivePosition` liefert für diese Zeilen kein `closed` (vermutlich `null` durch `acquirePositionMutationLock`, da die laufende Engine die Zeilen alle 60 s anfasst). Zu prüfen: Rückgabewert loggen; Sweeper innerhalb der Engine laufen lassen.

**Empfehlung nächster Schritt:** (a) Sweeper-Hook in den Realtime-Sync verlegen; (b) Ursache des Pro-Zyklus-Reservierungs-Leaks für `UNIUSDT short` finden (executeLivePosition-Abbruch vor Status-Write); (c) Index-Leser sollen IDs ohne Hash verwerfen bzw. JSON-TTL an Hash-TTL koppeln; (d) bis dahin operativ: die 48 Zeilen per Engine-internem Sweep oder Index-Bereinigung entfernen. Live-Kette (Audit-Retry, Deferred-Audit, Rollback-Cooldown) ist aktiv; seit 02:46 keine ungewollten Entries.

## Fortsetzungsstand — 2026-09-16 ~01:00 UTC (Claude-Sitzung: Entry-Churn behoben)

**Ursache des Churns der Live-Phase (PR #377, `main` 95392897, deployed):** Das Post-Entry-Audit war ein Einzelschuss; alle beobachteten Fehlschläge waren **Lese-Timeouts** (`[getPositions(fresh-snapshot)] Timeout after 2569/3940 ms`, `[getPositions(exact-protection-slot)] Timeout after 8000 ms`) unter der Request-Last nach einem Entry (Fill-Polling, drei Schutz-Orders, Aggregate-Reconcile). Ein geworfener Lesefehler wurde wie ein bewiesener Verstoß behandelt (`finalAdmission=null` → Rollback der gefüllten, bereits armierten Position, `entry_protection_contract_incomplete`); die Strategie qualifizierte erneut → 4 ATOM-Entries/11 min. Fix: (a) Audit-Lesen mit Backoff wiederholen (2/5/8/10 s, `CTS_POST_ENTRY_AUDIT_READ_DELAYS_MS`); (b) weiterhin nicht lesbar → Position bleibt **offen unter armiertem Schutz**, `post_entry_audit_deferred`, Halt für neue Exposure (`post_entry_audit_unavailable`) — keine Venue-Mutation ohne Evidenz; Rollback nur bei bewiesenem Verstoß; ein gescheiterter Initial-Aggregate-Reconcile erzwingt keinen Rollback mehr, wenn das Audit „safe" beweist. (c) Nach einem Protection-Rollback 15-min-Cooldown pro Symbol/Richtung (`live:entry-rollback-cooldown:<conn>:<SYMBOL>:<dir>`, Redis, prozessübergreifend), Block-Code `post_rollback_cooldown` im Interlock vor Step 5. Unit+Integration 313/2182.

**Offen — Pre-Fill-Zombies:** Zwei ATOM-Zeilen `live:bingx-x02:ATOMUSDT:short:signal_trailing:1789497385477:izfrv7` / `…:1789497402931:928v86` stehen seit 18:36 UTC auf `pending`, `orderId` leer, `executedQuantity 0`, kein `submissionState` — reservierte Slots aus der Churn-Phase, nie abgeschickt. Der Sync schreibt ihnen weiter `exchangeData.markPrice/syncedAt`, das UI zählt sie als offene Positionen (3 ATOM statt 1). `closeLivePosition` kann sie lokal finalisieren (`isPreFillWithoutExchangeHandle` + Connector `null`), aber der in Zeile ~17303 kommentierte „stuck-placement sweeper" **existiert nicht** (nur Simulated-Prozessor und Boot-Reconciliation, beide greifen nicht). Nächster Fix: Sweeper im periodischen Live-Sync — Status in {pending, placed, pending_fill, placed_unconfirmed}, `executedQuantity 0`, kein Venue-Handle (`hasSystemVenueHandle` false), älter als ~10 min → `closeLivePosition(conn, id, exitPrice, null, "placement_stuck_no_venue_handle")`; bis dahin die beiden Zeilen gezielt so schließen. Realtime-Progression-Budget; Reset-DB prozessübergreifend; geteiltes X02-Konto (CTS-G) — der Halt hebt sich nur in Momenten leerer Fremdbücher auf, dann handelt CTS-K-N live; eigener API-Key bleibt empfohlen.

## Fortsetzungsstand — 2026-09-15 ~20:20 UTC (Claude-Sitzung: Close-Fix, erste echte Live-Phase nach Halt-Aufhebung)

**Gemergt und deployed (PR #375, `main` 567bb305):** Ursache der Force-Close-Schleife bei adoptierten Zeilen: `closeLivePosition` erkannte als Venue-Handle nur `orderId`/`exchangeData.exchangePositionId`; Adoption speichert die Venue-Position aber als `exchangeData.positionId` → Close wurde als „external position protection" **übersprungen** (nie gesendet), Zeile blieb offen (`retry_at=0; class=unknown`), Schutz-Legs wurden jedes Mal neu konfiguriert (verdoppeltes Buch), max-hold Force-Close jeden Zyklus. Fix: `hasSystemVenueHandle()` (exportiert, getestet) + Standard-Backoff (`scheduleSystemCloseRetry`) im „unconfirmed"-Zweig. Adoption erfolgt nur nach Ownership-Beweis (Wasserzeichen oder vollständiges eigenes Drei-Order-Buch), daher legitim. Unit+Integration 312/312.

**Erste echte Live-Phase (18:20–19:48 UTC, ohne mein Zutun):** Nach dem BTC-Close war das Venue-Buch offenbar zweimal „leer" (auch CTS-G flat) → `reconcileEmptyBookProtectionHalt` hob den Halt regelkonform auf → Engine handelte wie konfiguriert: SUIUSDT short 8 (2×), ATOMUSDT short 3.25/3.25/3.27/6.04 (`signal_trailing`), alle gefüllt (Traces in `live_order_log`). Mehrere wurden sofort wieder geschlossen („Post-entry venue audit could not prove row TP/SL plus full-slot security protection") und neu eröffnet — **4 ATOM-Entries in 11 Minuten** (Churn). Um 19:48 setzte der Pre-Accumulation-Guard einen neuen Halt: `pre_accumulation_admission` / `authoritative_protection_snapshot_unavailable`. Stand: 1 ATOM short 3.27 `filled` mit vollem Schutzbuch (`exchange_control`), 2 ATOM-Zeilen `pending` mit executedQuantity 0 (zu prüfen: Zombies?), diverse closed. **Offen (Engine):** Entry→Audit-Fehlschlag→Close→Re-Entry-Churn braucht eine Sperre nach fehlgeschlagenem Protection-Audit (kein sofortiger Re-Entry auf demselben Symbol/Richtung); Ursache von `authoritative_protection_snapshot_unavailable` (Venue-Open-Order-Snapshot nicht abrufbar — Rate-Limit/VST?) klären.

**Entwarnung:** `/api/data/positions` vs. Ledger war nur ein Übergangszustand (Index-Entfernung wenige Sekunden nach Hash-Update), kein Bug.

## Fortsetzungsstand — 2026-09-15 ~16:35 UTC (Claude-Sitzung: adoptierte BTC-Position, festgefahrener Close)

Der Rogue-Entry `BTCUSDT SHORT 0.0001` (02:45, siehe oben) blieb auf der Venue offen; lokal als `adopted:BTCUSDT:short` mit `close_failed_exchange_unconfirmed: max_hold_time_exceeded … retry_at=0; class=unknown`. Beobachtet: (1) Engine hatte das Schutzbuch **verdoppelt** (6 eigene Reduce-Orders: 3 originale 02:45 + 3 neue 09:43 `system_close_fallback`); (2) Force-Close-Schleife ohne Backoff (~90 Journal-Zeilen/3 min, „MAX HOLD TIME exceeded … force-closing" / „Exchange close was not confirmed … durable delivery will be reconciled before the bounded retry"). Mit `scripts/reconcile-bingx-x02-protection-slot.ts` (Marker gesetzt, Services aus, Dry-Run → `--apply`) die 3 überzähligen eigenen Orders gecancelt (Report `.agent-logs/bingx-x02-protection-slot-2026-09-15T16-20-52-108Z.json`). Schleife lief danach weiter → Ursache nicht die Duplikate: `BingXConnector.closePosition` schließt korrekt (direkter Aufruf → Position weg, spätere Abfrage `101205 No position to close`), d. h. die Engine hat ihre Close-Versuche **gar nicht abgeschickt**, sondern auf die Reconciliation einer unbestätigten „durable delivery" (Close-Order des Rollbacks von 02:45) gewartet, die nie auflöst. Position per direktem Connector-Close (eigene Position, VST) geschlossen. Offen: Engine-Fixes für (a) Schutzbuch-Duplizierung beim Re-Arm (`system_close_fallback` muss vorhandene eigene Slot-Orders wiederverwenden/canceln), (b) festgefahrene Delivery-Barrier + fehlender Backoff (`systemCloseRetry` bleibt leer, `retry_at=0`), (c) Close-Pfad loggt keine Venue-Antwort (nur Entry-Traces im `live_order_log`).
...
**Live verifiziert (Produktion):** Momentaufnahme aller Overview-Flächen: offene/geschlossene Positionen, Orders, PnL über `positions/stats` und `tracking/overview` identisch. 2-Minuten-Reihe (23 Samples): Zykluszähler monoton, keine Nulls, kein Verschwinden, Status stabil, PnL bewegt sich nur mit Markpreis. Steuerung: `POST pause` (12 ms) → Zähler 45 s eingefroren, Status `paused` → `POST resume` → Zähler läuft weiter. Settings-Stress: 12 `PUT /api/settings` in Folge (`backupInterval` daily/weekly), alle 200, 21–170 ms, Endwert korrekt, Engine durchgehend `running`, null Journal-Fehler.

**Bewertet, bewusst nicht geändert:** Die Schwankung der Per-Zyklus-Trichterwerte ist Design (`stageSnapshotPolicy.semantics=last-observed-active-symbol-rows-with-explicit-freshness`); Real/Live-Rotation schreibt pro Symbol den Stand des aktuellen Ticks. Eine kumulative Alternative existiert in `statistics-tracker` (`strategies:<conn>:<type>:evaluated`), wurde nicht in die Overview aufgenommen.

**Offen (unverändert aus Vorsitzung):** `bybit-x03` main-enabled ohne Credentials (Operator-Entscheidung); Realtime-Progression reißt 30-s-Budget (~10×/h, erst Instrumentierung); Reset-DB stoppt nur Web-Prozess-Engines; `entry_protection_halt` bleibt durch geteiltes X02-Konto (CTS-G) unauflösbar — eigener API-Key für CTS-K-N empfohlen.

## Fortsetzungsstand — 2026-09-15 ~02:10 UTC (Claude-Sitzung: Audit, Live-Test, zwei Fixes)

**Gemergt und deployed:** PR #362 (Status-Flächen `trade-engine/health`, `startup-debug`, `system/verify-startup` lesen den kanonischen Redis-Katalog statt des Legacy-Dateikatalogs; vorher `/status=running` bei `/health=idle` mit Platzhalter-IDs, und `startup-debug` hätte eine Bybit-Platzhalterverbindung starten können). PR #363 (**Reset-DB löschte per FLUSHDB die gesamte DB 0 — auf diesem Host geteilt mit CTS-G (`cts-ga:*`, drei Python-Prozesse)**; jetzt `flushOwnedKeys()` mit geschützten Präfixen `CTS_REDIS_PROTECTED_PREFIXES`, Default `cts-ga:,cts-g:`, Batch-Löschung, Cache-Invalidierung; gilt für alle vier Reset/Flush-Routen). Deploy-Pfad wie dokumentiert: Installer scheitert am vorbestehenden `entry_protection_halt`-Gate, danach `service-control.sh restart`.

**X02 Prod-VST Soak (20 min exakt, 16 Zyklen, 4 Pfade × dca/block, 7 unbelegte Symbole, virtuelles Mindestvolumen):** eigener Lifecycle vollständig korrekt — `cleanupOwnedResiduals: exposureQuantity=0, controlOrders=0`, alle Zyklen `flat=true`, 64 eigene Orders sauber platziert/gecancelt/geschlossen, idempotentes „already gone (109400)" korrekt. Gesamtverdikt trotzdem FAIL: `baselineRestored=false`, weil ein **Dritter parallel auf demselben Konto handelt** (20 Fremdpositionsänderungen, 18 fremde Orders; XRPUSDT-SHORT-Residual nachweislich nicht von uns — ID nur in `residualSoakOpenOrderIds`, Erstellungszeit während unseres BTC/DOGE-Zyklus). Laut Dashboard-Manifest nutzt CTS-G dieselbe `bingx-x02`-Verbindung. Reports unter `/opt/cts-kn/.agent-logs/bingx-vst-soak-2026-09-15T00-47-42-910Z.json`.

**Live-Trade-Freigabe:** Operator-Intent steht bereits auf Live (`is_live_trade`, `live_trade_enabled`, `live_trade_requested` = true, `execution_mode=live`). Einzige Blockade ist der selbstheilende `entry_protection_halt` (`reconcileEmptyBookProtectionHalt` verlangt zweimal „leeres Buch"), der auf X02 wegen der Fremdpositionen nie erfüllbar ist. Nutzerwunsch „live trade always allowed" **nicht umgesetzt** (AGENTS.md; Hedge-Modus aggregiert Mengen pro Symbol/Seite → ein CTS-Close könnte fremde Menge schließen). Empfohlener Weg: eigener VST-Unterkonto/API-Key für CTS-K-N oder CTS-G von `bingx-x02` trennen; dann löst sich der Halt selbst.

**Gemessen, kein Handlungsbedarf:** Event-Loop frei (223 Probes/2 min, 0 Stalls, max 184 ms); Log-Volumen ~570 Zeilen/h ≈ 5 MB/Tag, Journal 250 MB mit Retention; App-RSS 1,2 GB (MemoryMax 3,9 GB), Redis 697 MB, Host 8,4 GB frei. Indikationszyklus ~45 s (min 33 s, max 97 s) bei `mainEngineIntervalMs=700` — IO-gebunden, nicht blockierend, Symbolverteilung über Zyklen gleichmäßig (Rotations-Bursts einzelner Zyklen sind kein Duplikat-Bug).

**Offen (mit Evidenz):**
- `bybit-x03` ist main-enabled ohne Credentials (`execution_mode=simulation`, Produktion verbietet Simulation): Connector-Fehler alle ~8 min, globaler Health-Rollup 26×/h `unhealthy`. Empfehlung: Verbindung deaktivieren (Operator-Setting, nicht stillschweigend geändert).
- `[CycleDiagnostic] realtime-progression exceeded 30000ms; continuing exhaustive work` ~10×/h: Realtime-Zyklus reißt sein Budget regelmäßig. Keine Stage-Timings exponiert (`/api/engine-metrics`, `/api/trade-engine/metrics` leer) → erst Instrumentierung, dann Optimierung; keine Änderung von Defaults auf Verdacht.
- Reset-DB prozessübergreifend: `stopAllProgressionsBeforeReset()` stoppt nur Engines im Web-Prozess; Scheduler/Direct-Trade laufen während eines Resets weiter. Sollte über den Wartungsmarker koordiniert werden.
- Kanonischer Server-Workspace jetzt vorhanden: `/workspace/CTS-K-N` (main, Dependencies installiert).
...
The official explicitly authorized X02 live-enabled reinstall used the sole-writer unit `cts-kn-sol-fd531f18-20260913.service`. Remote304 suites/2127 tests passed; compile succeeded in88s,42 pages and350 traces. The final production-init gate exited1 specifically on the existing X02 `entry_protection_halt`, so this is not a full live-readiness pass. Canonical `scripts/service-control.sh restart` restored all three application units AND the Redis-governor timer, preserving the protection halt. The complete post-deploy contract then passed at00:35:40UTC: health/schema/persistence/identity and authenticated continuity ticks; unauthenticated cron traffic correctly failed closed. No second installer/manual build overlapped.

Three sanitized samples00:36:37→00:43:23 show all three units active/NRestarts0, maintenance cleared, fresh Main/Direct heartbeats, Direct ticks168→597→1353, progression cycles2627→2630→2634 with zero failed cycles, and zero Direct errors over5min. Direct's persisted calculation was still from23:34:56, so after a new checkpoint the supported calculation endpoint was called with unchanged saved X02 settings. New calculation00:42:12 covers360192 sets/8 symbols/90h with367 valid, confirmed loaded by the live status surface. Own Direct open/opening/closed rows remain0. Preset exchange mode remains explicitly operator-disabled; Main/Signal/Direct requested live stay blocked by the same guard in every readiness surface. Root free space27.07GiB in the last sample; governor state normal, fresh lastRunAt, forkAllowed=true, overBudget=false, aofGrowthPressure=false. Its authoritative state file is `/var/lib/cts/instances/cts-kn/data/redis-memory-governor.json`.

Latest fresh signed X02 read00:44:07:2 positions,3 external orders,1 duplicate-shape group, both snapshot statuses successful. External positions/orders were not adopted, canceled or reconstructed. Required remaining order/accounting work: protection reconciliation;6 integrity mismatches in8 historical rows (missing fill/adjustment proof and missing originating-Set lineage), plus incomplete lifetime statistics after historical contribution pruning. Do not invent fills or clear the halt; no new authenticated order lifecycle pass or full production readiness is claimed. Use authoritative venue evidence and a separate verified checkpoint before any supported reconciliation. Existing exact-slot maintenance guards remain intact.

SOL request is complete as a historical comparison: Aug29–Sep12, windows3/6/12/24/48,1187200 diagnostic evaluations and7096320 Block-lane evaluations. A separate Aug1–Aug15 training matrix repeated1187200 evaluations. Full-ledger hashes are invariant across windows; effective net PF uses the selected Block lane or Base/DCA and includes modeled costs. No training candidate met cost stress plus42 closes, hence no candidate was evaluated on the reserved chronological holdout and no defaults were changed (window12 retained). Full executed matrices and an interactive report were saved as `CTS-K-N-SOL-Fenstervergleich.html` and `CTS-K-N-SOL-Fenster-Nachweise.zip`; prior SOL/DCA artifacts also remain saved. Dynamic Block recovery stages and exchange partial fills are separately tested, not claimed as a two-week venue replay. Protocol/results including sanitized runtime samples are in `docs/reports/20260913-sol-windows/`.

Verified checkpoints: local `/workspace/backups/CTS-K-N/20260913T001700Z-premerge-sol-windows` and `/workspace/backups/CTS-K-N/20260913T004605Z-before-published-handoff`; server `/var/backups/cts-kn/20260913T002627Z-before-sol-reinstall`, `/var/backups/cts-kn/20260913T003536Z-before-sol-service-recovery`, `/var/backups/cts-kn/20260913T004014Z-before-corrected-calculation`; final server `/var/backups/cts-kn/20260913T004617Z-final-sol-release`; permanent official rollback `/var/backups/cts/cts-kn/20260913T002629Z`. Final server/local checkpoint paths are also recorded in the durable session record `/workspace/backups/CTS-K-N/release-fe0a7425-state.json`. Each source checkpoint includes full available Git bundle, binary patch, status/HEAD, classified untracked archive/list, SHA256 verification and bundle verification; credentials/account payloads/Redis snapshots are excluded. Native persistence and official rollback remain untouched.


## Current continuation — 2026-09-13: SOL last-position windows, ready for publication

Canonical `/workspace/CTS-K-N`, branch `codex/sol-causal-block-recovery-20260912`. Local application correction is committed at `6814043990975025f2a6788bb245c0b522d68d7f`; the additional window harness/test and this handoff are pending commit. GitHub main and remote remain PR347 `fe0a7425e5525268bcd5a432c03e2611d10d79c2`. All earlier local branches were inventoried: the Overall Controls feature commit is included by PR347; this branch preserves that merged tree and the causal-PnL/Redis recovery delta. No open PR was found. The normal Git push lacks credentials; use the connected GitHub blob/tree/commit/PR flow, compare exact tree hashes, merge green, then official live-enabled X02 reinstall. That deployment and push/merge remain explicitly user-authorized.

Latest requested SOL diagnostic: Aug29–Sep12, fourteen complete UTC days; windows3/6/12/24/48, each237440 evaluations, total1187200 and7096320 Block-lane evaluations. Base-cost valid sets by window:3072/2391/1010/358/18; stress-cost:580/240/14/0/0. Complete Base and all Block ledgers are byte-hash invariant across windows; recent selection may change. The report now explicitly measures the selected effective Block (or Base/DCA) net PF, not a copied Base PF or the PositionCost admission coordinate. Every row checks finite values, count windows, summed net ledger, PF denominator and quantity bounds. These overlapping configuration observations are not a portfolio or venue fills.

Separate Aug1–Aug15 SOL training repeated the same1187200 evaluations. No candidate met both cost-stress qualification and42 closed trades; therefore zero candidates reached the reserved Aug15–Aug29 chronological holdout. The holdout candles are complete but NO holdout performance pass is claimed. No strategy/default threshold or existing saved setting changed; recent default stays12. Exact protocol, aggregate rows and hashes are in `docs/reports/20260913-sol-windows/`. Larger sample and independent positive net results remain required before default promotion. The historical Block replay still uses recovery stage1; dynamic recovery/stage2/partial fills remain separate executable lifecycle tests.

Final additional gates:304 suites/2127 tests, TypeScript and focused lint pass. Production application code is identical to the already-green6814043 build (42 pages/350 traces); subsequent edits are the offline diagnostic harness, regression and documentation. Full lint, source syntax, security (1729 files/0 findings), Kilo preflight (37 checks/schema108) and recreation verification (1721 files) also pass. Latest checkpoint before these docs: see `/workspace/backups/CTS-K-N` timestamped `before-window-handoff`.

Remote read-only00:07UTC: all three CTS-K-N services active with0 restarts, governor timer waiting,31.3GiB free. Direct is healthy with fresh heartbeat/progress and0 errors in5min,360192 evaluated/233 valid sets. Main runtime evidence and all scoped overview/progression endpoints answer200. Direct has0 own open/opening/closed positions. The pre-existing X02 `entry_protection_halt` remains; latest signed exchange observations before this continuation were3 positions/3–5 external orders. No external cancellation/adoption, protection deletion or new exchange lifecycle run occurred. Prior empty-account observations below are stale. Required remaining work: publish/merge exact checked tree; fresh server backup; sole-writer official reinstall; canonical recovery if existing halt fails final init; post-deploy contract, governor/disk/progression and fresh signed account monitoring; final exact handoff/checkpoints.


## Current continuation — 2026-09-12: SOL replay, Block quantities and Redis recovery

This entry supersedes older workspace, revision and pending statements below. Canonical checkout: `/workspace/CTS-K-N`. GitHub main and the deployed source are `fe0a7425e5525268bcd5a432c03e2611d10d79c2`, tree `e3c1dbb3e2a41afd73a04e62784f37a918bc2ec0`, from merged PR347. The default-off Overall Control Orders only feature, shared partial-fill allocation and readiness fixes are included. PR and merged-main GitHub smoke passed; Vercel main deployment became READY. Its browser preview requires Vercel login, so no interactive app-browser pass is claimed.

The latest user scope remains CTS-K-N only: push/merge and official live-enabled X02 Prod-VST reinstall are explicitly authorized. The final fourteen-day simulation is SOL only, with emphasis on Block volume ratios, partial fills, coordination, trailing strategies and performance. Preserve other projects and external orders. Never clear an unproven entry-protection halt manually or turn historical qualification into a fabricated live entry signal.

The follow-up delta corrects two demonstrated historical defects: higher-timeframe final OHLC values were consumed before candle close; quantity-weighted Block/DCA PnL used changing leg-price denominators instead of one initial notional. Current live partial-candle signals retain their intended behavior. Added costs follow actual leg notionals; full and recent admission coordinates use matching cost exposure while quantity ratios remain independent. Long/short numerical regressions failed before the fix and pass afterward.

Operational incident after the earlier successful deployment check: root storage filled, Main/Direct heartbeats stopped advancing, and the app still answered health200. The Redis AOF had grown to about 36.1 GB for a 926 MB dataset. The failed official install had stopped `cts-kn-redis-governor.timer`; the old public service controller restarted the application but omitted that timer. After a verified checkpoint, only the reproducible current webpack cache (1075 MiB) was removed, then a successful background AOF rewrite reclaimed about 33 GiB without deleting logical Redis/account data. Canonical CTS-K-N restart plus starting the existing governor restored fresh Main/Direct heartbeats and zero reported Direct errors. Root free space was 33.15 GiB in the final recovery probe. The follow-up restores the governor/legacy timer on normal start/restart and admits oversized-AOF compaction after 15 minutes while preserving memory/fork/loading/busy guards and retry backoff; normal cadence stays six hours.

Remote official reinstall of PR347 passed all 304 suites/2121 tests and the production build (42 pages/350 traces), then failed its final init gate on the pre-existing `entry_protection_halt`. Canonical recovery subsequently passed the full post-deploy contract, including authenticated minute ticks. Fresh X02 signed audits later observed external activity (initially 6 positions/43 external orders; the latest ten-sample retry retained 3 positions and 3–5 external orders). Both signed snapshot statuses were successful. CTS-K-N had zero own open rows; its eight legacy executed/closed ledger rows remain accounting-pending and lifetime contributions are pruned. No external orders were canceled or adopted. No full live/shared-control acceptance pass or complete historical accounting is claimed.

SOL replay: 2026-08-29 inclusive to 2026-09-12 exclusive, public BingX 5/15/30-minute candles, no gaps; seven strategy types, four entry/four exit tactics, seven timeframe combinations, both directions, TP0.5/1.0%, several SL/trailing grids, Block counts1–6, ratios0.1/0.5, configured maximum stages1/2, costs0.1/0.2%. It evaluated 237440 configurations,1419264 Block lanes,5859600 overlapping trade observations in38.779s;1024 configurations and3226 Block lanes passed the configured production filter. Heap checkpoints were about12MiB. All quantity, ledger, range, identity and finite-value assertions passed. The replay uses initial effective recovery stage1; dynamic recovery/pause and stage2 quantities are tested separately by executable lifecycle tests. Do not claim those transitions were simulated for fourteen days. No default promotion: the inspected period is not an independent holdout and overlapping configuration PnL is not a portfolio return. Main/Preset/Signal indicator-specific grids are outside this Direct replay. Full HTML and compressed per-configuration evidence are separate user artifacts.

The pending replay follow-up makes the recent-position admission window an explicit validated input from 3–50 positions, persists base, selected-Block and recent profit/loss/PF ledgers separately, and compares 3/6/12/24/48-position windows without allowing the window to change the complete historical ledger. Candidate qualification and ranking use the selected Block lane when configured, not the base row. A separate chronological 14-day holdout can be supplied, freezes candidates before evaluation and never promotes defaults. This follow-up is source-only and offline; it performs no Redis, credential, service or exchange writes. Publication remains contingent on a fresh complete gate and updated recreation manifests.

Follow-up verification: isolated full Jest JSON proves304/304 suites,2126/2126 tests; TypeScript, full ESLint, source syntax, shell syntax, secret scan (1726 files,0 findings), Kilo preflight (37 checks,schema108), recreation (1718 files), and final production build (42 pages,350 complete traces) pass. Publication/deployment of this follow-up remain pending until subsequent recorded evidence. Earlier attempted full Jest used a relative preload path that broke child fixtures; it was rerun with the absolute network-isolation preload and a durable JSON completion record. No failed fixture run is treated as a pass.

Verified local checkpoints: `/workspace/backups/CTS-K-N/20260912T203128Z-before-final-handoff`, `/workspace/backups/CTS-K-N/20260912T232343Z-before-causal-history-fix`, `/workspace/backups/CTS-K-N/20260912T233405Z-before-block-ledger-fix`, and `/workspace/backups/CTS-K-N/20260913T000618Z-midgate-active-worktree`. Remote checkpoints: `/var/backups/cts-kn/20260912T200831Z-predeploy-overall-controls`, `/var/backups/cts-kn/20260912T202640Z-before-service-recovery`, `/var/backups/cts-kn/20260912T233128Z-before-disk-recovery`, `/var/backups/cts-kn/20260912T233827Z-before-governor-recovery`. Official persistent backup is `/var/backups/cts/cts-kn/20260912T201150Z`; preserve its sensitive content and rollback state. All source checkpoints include complete available Git bundles, patches, classified untracked handling and verified SHA256 manifests; sensitive runtime data is excluded.

Remaining: finish follow-up gates, push/review/merge, deploy only green merged main through the official path, verify runtime and governor continuity again, then publish final exact handoff. VST may remain enabled under its existing ownership and admission guards; external exposure currently prevents the two-authoritative-empty-book proof required to retire the old halt. X01/Mainnet and Bybit remain read-only for this task.


<!-- CTS_LATEST_START -->
## Maßgeblicher Fortsetzungsstand — 2026-09-09, Bundle-Split abgeschlossen

Die aktuelle Integration liegt im verknüpften Worktree `/workspace/CTS-K-N-worktrees/vst25-live-results-20260908` auf Branch `codex/fractional-close-grid-20260909`; der abgeschlossene Revisionsstand ist `1774d0da006ef88e979550af469baacc02b09755`. Die externe Strategie ist aus diesem CTS-K-N-Bundle entfernt: Quellcode, Indikationsspur, Exits, Direct-Trade-/Live-Protection-Verknüpfungen, Einstellungen, APIs, Dashboard, eigenständige Reports, Verifikationsartefakte und Tests sind nicht mehr Bestandteil dieses Baums. Die native Trend-Evaluation bleibt erhalten.

Die Änderung ist lokal committed, aber nicht gepusht. Es wurden kein PR, kein Deploy, keine Reinstallation und keine Remote-/Exchange-/Redis-Mutation ausgeführt. Der separate externe Checkout wurde von dieser Arbeit nicht verändert.

Abgeschlossene Gates für diesen Baum: isolierte Vollprüfung `300` Suiten / `2096` Tests bestanden; fokussierte Regression `9` Suiten / `120` Tests bestanden; TypeScript, ESLint, Source-Syntax, `git diff --check` und Secret-Scan (`1731` Dateien, `0` Findings) bestanden; Recreation-Manifeste verifiziert (`1708` Projektdateien). Ein erster Vollsuite-Lauf hatte nur den bekannten timing-empfindlichen Dispatch-Test verfehlt; der vollständige Wiederholungslauf war grün.

Verifizierte owner-only Checkpoints: `/workspace/backups/CTS-K-N/20260909T133648Z-remove-external-precommit` und `/workspace/backups/CTS-K-N/20260909T133719Z-remove-external-postcommit`; die früheren Resume-Checkpoints bleiben ebenfalls erhalten. Vor einem weiteren Push/Merge/Deploy ist erneut ein Checkpoint zu erstellen und ausschließlich der geprüfte offizielle Veröffentlichungsweg zu verwenden.
...
- Implemented independent additive Block counts 1–6 / recovery levels 1–2,
  ...
  Settings invalidation and remaining indication stats/UI paths are now wired.
...
- The secondary X02 pulse service was subsequently repaired by concurrent work.
...
- ops/server-access-dashboard/ is now versioned as the read-only source for
  ...
  process and systemd resources, CTS-K-N probes and progression,
...
- Fixes: count signal-free successful cycles by actual errors; stage coverage uses fresh same-symbol intersection; restore Vercel's configured `vercel-build` alias with canonical hooks; dashboard cache age no longer slides forever and accounting completeness is explicit.
...
- Do not label production ready: push/green merge/remote reinstall/final remote soak still pending at this checkpoint. X01/Bybit remain read-only. No new mainnet or VST orders submitted in this continuation. Old VST soak failed baseline restoration because an external order changed; later own exposure was reconciled, not a passed original soak. The external strategy is outside this bundle and was not touched.
...
- `main` enthält PR322 (`0862ed419a47d763eea43bd443101286e19292a7`) und PR323 (`ef29134264048e71dc73d3cd98813c2acb70d3c7`). Remote `/opt/cts-kn` läuft exakt auf `ef29134264048e71dc73d3cd98813c2acb70d3c7`; `cts-kn`, `cts-kn-direct-trade` und `cts-kn-scheduler` sind aktiv, `/api/health` ist healthy/Redis connected, und der Reinstall-Backupmarker `/var/backups/cts/cts-kn/20260906T154303Z` ist verifiziert.
- Die Readiness-Aliase im Status-API sind kanonisch: X02 `running`, `execution_mode=live`, `live_execution_mode=live`, `live_trade_requested=1`, `live_trade_enabled=1`, `canPlaceRealOrders=true`, Credentials und durable coordination gültig, kein Blockcode. Overview filtert auf die eine Base-aktivierte Verbindung (X02) unter 13 gespeicherten Verbindungen. `bingx-8581b0cb8581` (X01) bestand einen lesenden `prod-live`-Connection-Test; X01/Mainnet/Bybit bleiben read-only.
- Realtime-Remote-Audit: 32/32 Symbolrotation, 32 unique, keine failed/stalled Symbole; ein neuer Zyklus wurde abgeschlossen und Folgearbeit lief weiter. Messstand: 32 Symbole, 46.24 Mio. Indikationen, 5 Indikations-/Strategiezyklen, 15 aktuelle kanonische Live-Positionen, 5.000 retained closed; native Trend 6.060 in der Breakdown-Anzeige.
...
- PR #326 was merged on 2026-09-07T18:51:33Z as merge commit `8d0f1f38ed0a21ab38faa01f0b6f23acb80e7a8e` on `main`; all required checks passed (`smoke`, `Vercel – cts-k-n`, `Vercel – cts-v`, and preview comments). `/vercel/share/v0-project` and the recovered canonical `/workspace/CTS-K-N` now point to merged `main` with clean worktrees, and the canonical remote was restored to GitHub. The feature branch remains locally retained for provenance; its remote branch was deleted by the merge. PR315 is separate and was not reused.
...
- Gates passed: unit `289/289` suites (`1,955` tests), integration `4/4` suites (`72` tests), full Jest `294/294` suites (`2,028` tests), TypeScript, ESLint, source syntax, production build with `349` complete traces, frozen pnpm install, secret scan (`1,686` files, `0` findings), and recreation verification (`1,678` files). Deployment-contract verification remains pending because no deployment URL is available locally; no endpoint was guessed.
- Focused coverage now includes recovery after the 109429 cooldown expires, no replay during cooldown, REST fallback, account-cache isolation, transient-row cleanup, and entry-halt dispatch.
- Verified owner-only checkpoints: `/workspace/backups/CTS-K-N/20260907T180031Z-x02-rejection-storm-fix`, `/workspace/backups/CTS-K-N/20260907T181758Z-x02-rejection-storm-fix-implemented`, `/workspace/backups/CTS-K-N/20260907T182831Z-x02-rejection-storm-fix-docs`, `/workspace/backups/CTS-K-N/20260907T183305Z-x02-rejection-storm-fix-prepublish`, `/workspace/backups/CTS-K-N/20260907T183514Z-x02-rejection-storm-fix-recovery-test`, `/workspace/backups/CTS-K-N/20260907T185240Z-post-pr326-merged-main-sync`, and `/workspace/backups/CTS-K-N/20260907T185416Z-merged-main-handoff-docs`; SHA-256 manifests and recorded `git bundle verify` passed.
- Publication passed through PR #326. The merged-main runtime gate is blocked because `/workspace/.network-clients/activate-cts.sh` and the managed Chisel listener/SSH configuration are absent in this VM. Per repository safety rules, no ad-hoc tunnel was substituted. No remote reinstall, service, order, Redis, or exchange mutation was made; remote health, row growth, statistics, and owned-order reconciliation remain unverified. X01/Mainnet/Bybit remain read-only, and no production-readiness claim is valid.

## Session 2026-09-08 — merged-main remote update attempt
...
## CTS-K-N remote recovery checkpoint — 2026-09-09 01:40 UTC

Canonical dirty `/workspace/CTS-K-N` remains preserved at 2041787b; all 34 older diffs are accounted for in docs/LEGACY-INTEGRATION-20260909.md. Active worktree `/workspace/CTS-K-N-worktrees/vst25-live-results-20260908` now branches `codex/pr332-runtime-acceptance-20260909` from merged/deployed PR332 main fdd33ed14b135a5c7808463e3ae5bb336af1ccb4. Main smoke and official reinstall passed; rollback `/var/backups/cts/cts-kn/20260909T011708Z`; full protected backup `/var/backups/cts-kn/20260909T0030-pre-redis-release/VERIFIED`; latest local edit backup `/workspace/backups/CTS-K-N/20260909T013515Z-pre-live-settings-projection-fix`.

Completed: 30 unused build-cache files removed (5330003422 bytes); guarded legacy unsubmitted-X02 retirement Exit0 (241870 operations; Redis used 3992661648→773217360 bytes; sampled executed hashes unchanged); AOF compaction OK (10892367760→467857440 bytes, same 51152 logical keys). All services restored afterward, NRestarts0. Server health/status/logs/settings/live-summary HTTP200. Requested X02 Live ON and 25-symbol basket persisted; canonical runtime gate truthfully shows the old entry_protection_halt. Lifetime 300691 failures are historical, not a new failure delta.

New concrete issue under follow-up: settings GET returned stale nested/flat OFF flags even when canonical connection Live ON. GET-only canonical projection added; 3 route tests pass, including canonical OFF precedence and no writes. Full follow-up gates/publication/deploy still pending. Original release tests were local isolated and remote 300 suites/2101 tests/Exit0; never rerun unisolated local tests. Use empty env plus scripts/test-network-isolation.cjs. Full browser stats acceptance remains blocked by the browser tool's security response; no URL/transport workaround.

Current long task: systemd unit cts-kn-pr332-vst25-soak executes the isolated merged fdd33ed1 harness, 25 symbols/25 cycles/40 min/200 planned cumulative submissions, virtual-only <=25 USD position notional, strict ownership/baseline restoration. Results pending under /var/tmp/cts-kn-vst25-20260908. Wrapper stops all three CTS-K-N services and restores in EXIT; The external strategy is outside this bundle. Successful cleanup is required before the separate empty-book halt audit may clear the known rollback halt using two fresh authoritative snapshots, canonical admission and Redis WATCH/owned lease. Never blindly DEL the halt or lower PF/ownership/protection guards. Persist final measured soak, API/settings fix, HTML and Git/remote revision evidence before handoff.

### Current order-counter correction — 2026-09-09

Working tree `/workspace/CTS-K-N-worktrees/vst25-live-results-20260908`, branch `codex/fractional-close-grid-20260909`, base `ab1133226744e93864f4ad51f26152cd920a259e`. A verified checkpoint is `/workspace/backups/CTS-K-N/20260909T052609Z-pre-live-dispatch-fix`. Current edits add `lib/live-dispatch-outcome.ts`, classify expected Live coordination skips as `deferred`, persist per-symbol/aggregate deferred counts, and keep terminal `failedToOpen` separate. Overview UIs now label lifetime venue errors/rejects separately from current dispatch errors, blocks, and deferrals. `reconcileLivePositions` now calls bounded `pushStep` instead of unsafe `pos.progression!.push`, fixing persisted legacy positions without progression arrays. Targeted tests: 2 suites / 217 tests passed including the new classifier; TypeScript and `git diff --check` passed. PR/merge/reinstall and remote post-fix evidence are pending; keep X02-only lifecycle scope and the existing entry-protection halt until fresh audit gates pass.

### VST25 first attempt resolved diagnosis — 2026-09-09 01:45 UTC

The preceding in-progress note is superseded: fdd33ed1 VST25 attempt failed at the first BTC post-close quantity check, after 8 actual order submissions. Entry+accumulation+full close filled, trailing replacements/stale-update rejection succeeded up to final security cleanup. A pre-close cached 0.0002 BTC snapshot was reused after a fully filled close and falsely triggered excess-owned-quantity rejection. Native request timestamps in the private report establish cache reuse; no fresh positions HTTP request occurred between the close settlement and this failure. CleanupComplete=true, owned residual exposure=0/control orders=0, empty baseline restored; all three services active afterward. Do not claim this as external interference or a passing 25-cycle run. The old halt remains, and no audit-clear was attempted.

Active follow-up adds lib/fresh-position-snapshot.ts to wait through cached/shared-inflight/pre-audit position snapshots, fail on provider errors, preserve normal API pacing, and enforce a bounded timeout. Wired to production connection-wide protection audit and the VST exact-quantity wait, retaining all ownership/excess guards. Fresh edit checkpoint 20260909T014137Z-pre-fresh-position-audit-fix. New merge/reinstall/rerun, final HTML and exact gate evidence remain pending.
...
## 2026-09-13 Recalculation fix — verified remote handoff

PR #350 merged as `99be53d9de4e30bdf0acaefb4914ac3dc4dd5953`, tree `3a318af50e05fc0d3f203e00290df4f233927946`, identical to local tested commit `0851529a`. Both GitHub Dev Preview Smoke runs passed (PR 34730108249; main 34730208027). Final local and remote suites: 304/304, 2,135/2,135 tests. Full lint, typecheck, syntax, security, preflight and recreation verification passed. Local build passed on its stable second attempt after documentation changed during the first; remote build passed with 42 pages, 350 traces, 87-second compile. Vercel status contexts were still pending when inspected; no browser/UI acceptance pass is claimed.

Official live-enabled reinstall deployed `99be53d9` through unit `cts-kn-recalc-20260913.service`. Its final init exited 1 specifically on the existing X02 `entry_protection_halt`; all artifacts and tests passed. After a verified checkpoint, canonical service-control recovery restored app, scheduler, Direct-Trade and the Redis governor timer without changing the protection gate. The complete post-deploy contract passed at 01:28:12 UTC. Private installer/recovery logs remain under `/var/tmp/cts-kn-recalc-20260913/`.

The processor automatically retained its pending recalculation across the abandoned 330-second publisher lease from the installer's first start (TTL observed 5 seconds at 01:32:43). No lease was deleted and no manual Calculate/settings request was sent. After expiry it completed 8/8 symbols and 360,192 evaluations, publishing 250 valid configurations at 01:34:59.539 UTC. At 01:36:03 it remained ready, with 1,391 ticks, zero five-minute processor errors, and healthy progress. All three services remained active with zero restarts, governor active, maintenance cleared. Progression advanced to 2,698 successful cycles / zero failed cycles; overview covered 25 symbols with Base/Main/Real/Live stage presence.

Outstanding: X02 live admission consistently remains blocked by `entry_protection_halt`; Preset exchange mode remains operator-disabled. The eight-row historical ledger still has six relationship mismatches and incomplete lifetime accounting. Current lineage construction already preserves the exact originating Set key, but old metadata/fill gaps require a proof-backed repair; the legacy repair script cannot supply missing venue fill evidence. Signed reads at 01:29:11 UTC succeeded with 5 positions and 13 orders, all 13 classified external. This differs from the earlier 3/3 observation: no baseline-restoration or authenticated lifecycle pass is claimed. No external cancellation/adoption, fabricated fill/lineage, safety-gate deletion or defaults change occurred.

Operational read-model caution: engine-states intentionally serves an expired snapshot while refreshing in the background. Its embedded heartbeat age belongs to snapshot creation; a first sparse GET must not be treated as proof of a current heartbeat. Direct-Trade status reads and repeated progression samples independently confirmed current processing.
