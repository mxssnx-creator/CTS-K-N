...
| Date | Changes |
|------|---------|
| 2026-08-08 | Intensive direct-trade verification: ran full recovery soak (run-direct-trade-recovery-soak.mjs) — lease takeover, settings persistence, open-position retention across Stop and SIGKILL+restart, worker lease adoption, controlled stop all pass. Matrix calibration (test-direct-trade-matrix.cjs, 3 symbols) confirms 177,408 evaluated sets, TP ratios [4,8,12,14], 12/6/300 position limits, 99MB heap, projectedDefault32SymbolSets=1,892,352 matching spec. All 953 tests (139 suites) pass; typecheck, lint, dev-preview smoke check all green. |
