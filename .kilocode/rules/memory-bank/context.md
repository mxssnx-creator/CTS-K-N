...
The owner-only environment inventory now provides the managed endpoint, fingerprint, auth, proxy, and SSH inputs, and a process-local Chisel client was restored at `/workspace/.network-clients/chisel` (1.12.0, mode 0700). Transport validation still fails closed: `CTS_CHISEL_PROXY` is a port-only token rather than a complete HTTP/SOCKS5 URL, and `CTS_SSH_KNOWN_HOSTS_CONTENT` is a single unparsable token with no strict `[127.0.0.1]:2222` entry. No activation, pinned SSH banner, direct public SSH, remote, service, Redis, exchange, deployment, or order mutation occurred.
