...
- Trend: zusätzliche EMA-Paare und Bestätigungen; Break: zusätzliche vorherige Ranges/Noise/Bestätigungen. Standardmatrix über vier Timeframes: Trend 72 Auswertungen / 144 gerichtete Sets; Break 48 / 96. Neue Kombinationen erhalten eindeutige IDs, Legacy-Basis bleibt erhalten. Kein als optimal oder profitabel zertifizierter neuer Default.
...
Current long task: systemd unit cts-kn-pr332-vst25-soak executes the isolated merged fdd33ed1 harness, 25 symbols/25 cycles/40 min/200 planned cumulative submissions, virtual-only <=25 USD position notional, strict ownership/baseline restoration. Results pending under /var/tmp/cts-kn-vst25-20260908. Wrapper stops all three CTS-K-N services and restores in EXIT; CTS-G is untouched. Successful cleanup is required before the separate empty-book halt audit may clear the known rollback halt using two fresh authoritative snapshots, canonical admission and Redis WATCH/owned lease. Never blindly DEL the halt or lower PF/ownership/protection guards. Persist final measured soak, API/settings fix, HTML and Git/remote revision evidence before handoff.
