    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.25fr_1fr]">
        <Card className="overflow-hidden p-4">
          ...
          <p className="mt-2 text-[11px] text-muted-foreground">PF basis: summed positive/negative ratio-weighted net PnL ({calculation.pfBasis || "aggregate_ratio_weighted_net_pnl"}); it is not an average of row PFs or TP ratios. The TP means (*) are diagnostics only. Target volume: Count {visibleRows[0]?.blockCount || 0}, {visibleRows[0]?.blockIncrementSteps || 2} additive recovery levels, {Number(visibleRows[0]?.blockCalculatedVolumeMultiplier || 1).toFixed(2)}×; realized average: {Number(visibleRows[0]?.blockRealizedVolumeMultiplier || 1).toFixed(2)}×. Best market exit is hindsight-only.</p>
        </Card>
        <Card className="p-4">
