...
const PRESET_MULTIPLIERS = [1.0, 1.5, 2.0, 3.0]
...
export function VolumeConfigurationPanel({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Signal-specific Main Trade Volume Factor */}
      <div className="space-y-3 rounded-lg bg-slate-50 dark:bg-slate-900/30 p-4">
        <div className="flex items-center justify-between">
          <div>
            <Label className="text-sm font-medium">Signal Volume</Label>
            <p className="text-xs text-muted-foreground">Applied once to Signal-originated Main orders; Base coordination remains 1.</p>
          </div>
          <Badge variant="secondary" className="text-xs">
            {signalVolumeFactor.toFixed(2)}x
          </Badge>
        </div>
        <Slider
          value={[signalVolumeFactor]}
          onValueChange={(value) => onSignalVolumeChange(value[0])}
          min={1}
          max={10}
          step={0.1}
          className="w-full"
        ...
        <div className="flex gap-2 flex-wrap">
          {PRESET_MULTIPLIERS.map((mult) => (
            <Button
              key={mult}
              variant={signalVolumeFactor === mult ? "default" : "outline"}
              size="sm"
              className="text-xs"
              onClick={() => onSignalVolumeChange(mult)}
              disabled={slidersDisabled}
            >
              {mult}x
            </Button>
