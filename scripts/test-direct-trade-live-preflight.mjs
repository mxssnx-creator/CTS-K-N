  ...
  try {
    ...
    const started = await request("/api/trade-engine/direct-trade", {
      ...
      body: JSON.stringify({
        ...
        symbolCount,
        symbols,
        // The worker must replace this paper range with its exact live 48h
        // warmup. Keeping it at 60 proves the boundary is not accidental.
        historyHours: 60,
        timeframes: ["1m"],
