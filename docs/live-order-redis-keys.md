...
| Per-symbol order counters | `live_orders_by_symbol_v2:{connectionId}` hash fields in `{SYMBOL}:{long|short}:{placed|filled|failed}` format. The version separates corrected side-independent counters from retained v1 forensic data. |
