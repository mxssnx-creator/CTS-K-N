...
data={'generatedAt':source['generatedAt'],'release':extra.get('release','fdd33ed14b135a5c7808463e3ae5bb336af1ccb4'),
      ...
      'runtime':runtime,'firstSoak':initial,'latestSoak':soak,'extra':extra}
...
<section><h2>Native Trend-Prüfung</h2><p>Die verbleibende Trend-Auswertung nutzt die nativen EMA-, Spread- und Bestätigungsparameter. Diese Messung dokumentiert die technische Ausführung; sie ist kein unabhängiger Profitabilitätsnachweis.</p></section>
...
<section><h2>Grenzen dieser Abnahme</h2><ul><li>Produktionsreife setzt einen vollständig bestandenen VST-Lauf, geklärten Schutzstatus, Browser-Abnahme und nachgewiesene Laufzeitstabilität voraus.</li><li>Ein positives Gesamt-PnL wird nicht durch zusätzliche Orders oder schwächere Mindestwerte erzwungen.</li><li>Die früheren 14-Tage-DCA-Auswertungen bleiben retrospektive Berichte. Sie ersetzen keinen neuen Holdout für Trend.</li><li>X01/Mainnet, Bybit und fremde Handelsbestände wurden nicht verändert.</li></ul><p><a href="data.json" download>Vollständige Messwerte als JSON</a> · <button onclick="window.print()">Drucken / als PDF speichern</button></p></section>
<footer>Release: <code>@@RELEASE@@</code><br>Die fortlaufende Projektkoordination ist in docs/PROJECT-CONTINUATION.md und der Projekt-Memory-Bank dokumentiert. Bericht ohne externe Skripte, Fonts oder Datendienste.</footer></main></body></html>'''
...
print(json.dumps({'report':str(OUT/'report.html'),'bytes':len(doc.encode()),'externalScripts':0}))
