...
export async function readConnectionDetails(definition, id, statusRow) {
  const now = Date.now();
  ...
  const refreshOverview = !cached || now - cached.overviewAt >= PROJECT_DETAIL_REFRESH_MS;
  const overviewRequest = refreshOverview
  ...
  connectionDetailCache.set(definition.id + ":" + id, {
    ...
    overviewAt: refreshOverview && overview.ok ? now : cached?.overviewAt || 0,
  });
