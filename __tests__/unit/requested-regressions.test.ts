...
describe("requested regression guardrails", () => {
  ...
  test("historic and indication CPU loops use responsive default scheduling quanta", () => {
    ...
    expect(strategy).toContain("LIVE_DISPATCH_HARD_MAX_PER_SYMBOL = 4")
    expect(strategy).toContain("planLiveDispatchCandidatesFairly(")
    expect(strategy).toContain("Physical exchange/paper side effects use a bounded durable")
    expect(strategy).toContain("persistedDispatchCursor")
    expect(strategy).toContain("`s:${symbol}:dispatch_cursor`")
