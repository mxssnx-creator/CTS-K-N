...
describe("requested regression guardrails", () => {
  ...
  test("startup and diagnostics never substitute unscoped market data", () => {
    ...
    expect(dump).toContain("scanRedisKeys(client, `market_data:${id}:*`")
    expect(dump).not.toContain("scanRedisKeys(client, `market_data:*`")
  })
  ...
  test("Preset UI exposes persisted historical diagrams, defaults, and the correct shared-engine assignments", () => {
    ...
    expect(page).toContain("Target volume increases additively from the immutable general volume")
    ...
    expect(optimizer).toContain("blockMaxStack: BLOCK_COUNT_MAX")
    expect(optimizer).toContain("blockVolumeRatio: 1")
    expect(optimizer).toContain("blockProfitFactorRatio: 1.1")
    expect(optimizer).toContain("blockIncrementSteps: BLOCK_INCREMENT_STEPS_MAX")
  ...
  test("Direct-Trade live control orders persist and reconcile one stable economic intent", () => {
    ...
    const canonical = read("lib/direct-trade-canonical-order.ts")
    ...
    expect(gateway).toContain("executeDirectTradeCanonicalOrder")
    expect(canonical).toContain("const settlement = entrySettlement(position, fill)")
    expect(canonical).toContain("beginDirectOrderControl")
    expect(canonical).toContain("directTradeCanonicalPositionId")
    expect(canonical).toContain("snapshot = await executeLivePosition(")
    expect(canonical).toContain("requestedDcaStep: dcaStep || undefined")
  ...
  test("block overlays completed-position counts and active-position exposure at Real stage", () => {
    ...
    expect(source).toContain("blockMaxStack:    6")
    ...
    expect(liveStage).toContain("targetSatisfied:")
  ...
  test("block pause count ratio is persisted and clamped for strategy settings", () => {
    ...
    expect(section).toContain("blockMaxStack:    6")
    expect(section).toContain("max={6}")
  ...
  test("historic and indication CPU loops use responsive default scheduling quanta", () => {
    ...
    expect(strategy).not.toContain('persistUnavailableDispatch("connector_unavailable"')
    expect(strategy).toContain("Always dispatch qualifying Sets through executeLivePosition")
    expect(strategy).toContain("handles the null connector gracefully (status=\"error\", blockCode set)")
  ...
  test("status-all derives running state from process-independent Redis runtime evidence", () => {
    ...
    expect(source).toContain('live_trade_enabled: canonicalLiveEnabled ? "1" : "0"')
    expect(source).toContain('live_trade_requested: canonicalLiveRequested ? "1" : "0"')
    expect(source).toContain('execution_mode: canonicalExecutionMode')
