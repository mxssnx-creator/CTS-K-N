...
describe("production installation and Kilo deployment contract", () => {
  ...
  it("keeps the canonical host installer fail-closed and complete", async () => {
    ...
    expect(bootstrap).not.toContain("chmod 750 scripts/install.sh")
    expect(bootstrap).toContain("Changing a tracked script's mode")
    ...
    expect(installer).toContain("upsert_env LIVE_ORDER_CONNECTION_IDS bingx-x01,bingx-x01-futures,bingx-x02,bingx-x02-vst-futures,bybit-x03,bybit-x03-unified")
    ...
    expect(installer).toContain("Global simulation is active; only leased BingX X02 Prod-VST Direct placement is enabled")
    expect(installer).toContain('[[ "${direct_x02_connection_ids,,}" == "bingx-x02" ]]')
    expect(installer).toContain("upsert_env DIRECT_TRADE_LIVE_ORDER_PLACEMENT 1")
    expect(installer).toContain("upsert_env DIRECT_TRADE_LIVE_ORDER_PLACEMENT 0")
    expect(installer).toContain("upsert_env DIRECT_TRADE_LIVE_CONNECTION_IDS bingx-x02")
    ...
    expect(installer).toContain('BingX X01 (write-eligible after all runtime gates)')
    expect(installer).toContain('Bybit X03 (write-eligible after all runtime gates)')
    expect(installer).toContain('Main/Preset/Signal writes remain allow-listed to BingX X01/X02 and Bybit X03 identities')
    ...
    expect(productionInit).toContain("BingX X02 Prod-VST Main Trade remains intentionally operator-stopped")
    expect(productionInit).toContain('runtime?.reason === "operator-stopped"')
    expect(productionInit).toContain("runtime?.operatorStopped === true")
  ...
  it("ignores only inactive immutable release and rollback snapshots", async () => {
    ...
    try {
      ...
      expect(() => execFileSync("bash", args, {
        ...
        encoding: "utf8",
        stdio: "pipe",
      })).toThrow(/already owns service name/)
