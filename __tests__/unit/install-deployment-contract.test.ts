...
describe("production installation and Kilo deployment contract", () => {
  ...
  it("keeps the canonical host installer fail-closed and complete", async () => {
    ...
    expect(installer).toContain("Live trading installation is enabled; credentials will be validated by runtime readiness gates")
    expect(installer).toContain("CTS_LIVE_TRADING_CONFIGURED 1")
    expect(installer).toContain("if ! pnpm exec jest")
    ...
    expect(envExample).toContain("CTS_LIVE_TRADING_CONFIGURED=")
    expect(envExample).toContain("Missing credentials never prevent installation")
    expect(envExample).toContain("installer always configures live-capable mode")
    expect(envExample).toContain("Never hardcode exchange credentials")
