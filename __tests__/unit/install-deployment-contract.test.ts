...
describe("production installation and Kilo deployment contract", () => {
  ...
  it("keeps the canonical host installer fail-closed and complete", async () => {
    ...
    expect(installer).toContain("No exchange credentials were supplied; installation continues with live trading capability enabled but real orders blocked until credentials are configured")
    expect(installer).toContain("CTS_LIVE_TRADING_CONFIGURED 0")
