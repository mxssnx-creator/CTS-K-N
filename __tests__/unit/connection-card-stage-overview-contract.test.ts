...
describe("connection card stage overview contract", () => {
  test("the stats route builds one coordinated cycle/open overview with real coverage", () => {
    ...
    expect(route).toContain("freshSymbols >= expectedStageSymbolCount")
    expect(route).toContain("aggregateCompleteFreshRowField")
    expect(route).toContain("coverage.complete ? detail.length : 0")
    expect(route).toContain("const stageEvaluated = useCross ? symEvaluated : 0")
    expect(route).toContain("return samples > 0 ? total : 0")
  ...
  test("the card consumes the canonical stats snapshot once and clears prior connection state", () => {
    ...
    expect(card).toContain("stratReal:  nonNegativeMetric(rows.real?.valid ?? strat.real)")
    expect(card).toContain("Current Logical Stage Rows &amp; Open Positions")
    expect(card).toContain('isLive ? "pos" : "rows"')
