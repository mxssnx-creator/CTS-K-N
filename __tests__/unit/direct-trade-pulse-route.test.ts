...
describe("Direct-Trade entry pulse", () => {
  ...
  test("refreshes compact current 1m/10m/15m signal lines without loading the historic config grid", async () => {
    ...
    // 7 non-empty timeframe combinations × 2 directions × 1 tactic × six
    // default-enabled lineages (standard/fixed/auto/combination/inverse/protection).
    expect(payload.signalsEvaluated).toBe(84)
