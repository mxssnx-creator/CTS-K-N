...
describe("migrations 080–107 exact Set indexes and current engine defaults", () => {
  ...
  test("backfills lifetime/active/closed indexes, O(1) totals, and canonical Previous windows", async () => {
    ...
    try {
      ...
      await client.set("direct_trade:connection:conn-ledger:state", JSON.stringify({
        connectionId: "conn-ledger",
        minVolFactor: 5,
        volumeFactor: 5,
      ...
      expect(await client.hget("settings:connection_settings:conn-ledger", "blockProfitFactorRatio")).toBe("1.1")
      expect(await client.hget("settings:connection_settings:conn-ledger", "blockIncrementSteps")).toBe("2")
      ...
      expect(await client.hget("connection_settings:conn-ledger", "preset_volume_factor")).toBe("1")
      expect(await client.hget("connection_settings:conn-ledger", "control_orders")).toBe("1")
      expect(await client.hget("connection_settings:conn-ledger", "control_orders_enabled")).toBe("1")
      expect(await client.hget("connection:conn-ledger", "volume_factor")).toBe("1")
      expect(await client.hget("connection:conn-ledger", "live_volume_factor")).toBe("1")
      expect(JSON.parse(
      ...
      expect(JSON.parse(String(await client.get("direct_trade:connection:conn-ledger:state")))).toMatchObject({
        minVolFactor: 0.1,
        volumeFactor: 0.1,
  ...
  test("raises the schema-89 stale 0.8 default to the current 1.15 default and preserves higher downstream settings", async () => {
    ...
    try {
      ...
      expect(await client.hget("connection_settings:conn-v90", "mainProfitFactor")).toBe("1.1")
      expect(JSON.parse(String(
