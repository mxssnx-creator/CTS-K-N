...
describe("QuickStart timeout and maximum-symbol contract", () => {
  ...
  test("keeps the production harness aligned with the 128-symbol stress reference", () => {
    ...
    expect(QUICKSTART_UI_MAX_SYMBOLS).toBe(1_000)
    expect(quickstartSection).toContain("QUICKSTART_UI_MAX_SYMBOLS")
    expect(runner).toContain("const UI_MAX_SYMBOLS = HIGH_SCALE_SYMBOL_STRESS_TARGET")
    expect(runner).toContain('process.argv.includes("--max-symbols")')
    expect(soak).toContain('rounds % 10 === 0 && RUNTIME_MODE !== "production"')
    expect(uiVerifier).toContain("const UI_MAX_SYMBOLS = 128")
    expect(uiVerifier).toContain("PROD_UI_SYMBOLS_JSON")
    expect(uiVerifier).toContain("configuredUiSymbols()")
    expect(uiVerifier).toContain("RESUME_PROGRESSION_TIMEOUT_MS")
    expect(uiVerifier).toContain("Math.min(configuredResumeTimeout, 30 * 60_000)")
    expect(uiVerifier.match(/timeoutMs: PROGRESSION_TIMEOUT_MS/g)?.length).toBeGreaterThanOrEqual(1)
    expect(uiVerifier.match(/stats\?view=runtime/g)?.length).toBeGreaterThanOrEqual(2)
    expect(uiVerifier.match(/timeoutMs: 15_000/g)?.length).toBeGreaterThanOrEqual(2)
    ...
    const inlineAudit = source("scripts/run-production-inline-ui-audit.mjs")
    expect(inlineAudit).toContain('symbolSource = "deterministic-simulated-fallback"')
    expect(inlineAudit).toContain("PROD_INLINE_AUDIT_REQUIRE_LIVE_DISCOVERY")
    expect(inlineAudit).toContain("PROD_UI_SYMBOLS_JSON: JSON.stringify(symbols)")
    expect(inlineAudit).toContain('ALLOW_LIVE_ORDER_PLACEMENT: "0"')
    expect(inlineAudit).toContain('FORCE_SIMULATED: "1"')
  })
})
