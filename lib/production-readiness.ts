  ...
  const backend =
    typeof redisBackendGetter === "function"
      ...
      : typeof (RedisDb as any).getRedisBackend === "function"
        ...
        : "unknown"
  const inlineRedisAllowed = process.env.ALLOW_PROD_INLINE_REDIS === "1" || process.env.ALLOW_INLINE_REDIS_LIVE_TRADING === "1"
  if (process.env.NODE_ENV === "production" && backend === "inline-local" && !inlineRedisAllowed) {
    missingFields.push({
      field: "redis_backend",
      expected: "redis-network",
      actual: backend,
      details: { reason: "inline-local Redis is not permitted for production engine starts without ALLOW_PROD_INLINE_REDIS=1" },
    })
  }
