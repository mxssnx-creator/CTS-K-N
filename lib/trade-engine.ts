...
export class GlobalTradeEngineCoordinator {
  ...
  async startEngine(connectionId: string, config: EngineConfig, options: StartEngineOptions = {}): Promise<boolean> {
    ...
    if (isServerlessWorker && !explicitForegroundAllowed) {
      if (!serverlessOwnsEngine && !forceLocalTakeover) {
        console.warn(
  ...
  async startMissingEngines(connections: any[]): Promise<number> {
    try {
      ...
      // Every explicitly eligible connection is independently runnable in
      // development and production. A former development-only one-engine cap
      // silently pinned work to X01, made X02 progress appear stuck, and hid
      // cross-connection defects. Resource safety is enforced by the shared
      // runtime concurrency profile and per-engine bounded work queues rather
      // than by dropping an operator-selected connection.
