...
function calculateRedisMaintenanceAdmission({ policy, availableBytes, usedBytes, rssBytes = 0, persistence = {}, now, lastPurgeAt = 0, lastAofAttemptAt = 0 }) {
  ...
  // A six-hour cadence is too slow when high-rate overwrites expand the AOF
  // tens of times beyond the retained dataset. Keep the same fork admission
  // and failed-attempt backoff, but compact excessive growth earlier.
  const aofGrowthPressure = Number(persistence.aof_current_size) > Math.max(1024 * MIB, (Number(usedBytes) || 0) * 4)
  const aofCooldownMs = (aofGrowthPressure ? 15 : 6 * 60) * 60 * 1000
  return {
    loading,
    busy,
    forkReserveBytes,
    ...
    aofRewriteAllowed: forkAllowed && now - lastAofAttemptAt >= aofCooldownMs,
  }
}
